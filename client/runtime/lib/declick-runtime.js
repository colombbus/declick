(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("declick-runtime", [], factory);
	else if(typeof exports === 'object')
		exports["declick-runtime"] = factory();
	else
		root["declick-runtime"] = factory();
})(window, function() {
...
/***/ "./node_modules/js-interpreter/lib/index.js":
/*!**************************************************!*\
  !*** ./node_modules/js-interpreter/lib/index.js ***!
  \**************************************************/
...
var _interpreter2 = _interopRequireDefault(__webpack_require__(/*! ./interpreter */ "./src/interpreter.js"));
...
// Private properties
//let _log = null;
var _classes = {}; // classes without constructor

var _classStructures = {};
var _instances = {};
var _interpreter = null;
var _stored = false;
var _createdObjects = []; // Private methods

var _toNativeData = function _toNativeData(data) {
  var result;
...
var _toInterpreterData = function _toInterpreterData(interpreter, data) {
  var result;
  ...
  if (data instanceof Array) {
    ...
    if (data.className != null && _classStructures[data.className] != null) {
      ...
      var _result = interpreter.createObject(_classStructures[data.className]);
...
var _getMethodWrapper = function _getMethodWrapper(interpreter, method) {
  return function () {
    // transform data from interpreter into actual data
    var args = [].concat(Array.prototype.slice.call(arguments)).map(function (argument) {
...
var _toInterpreterClass = function _toInterpreterClass(interpreter, AClass) {
  // 1st prototype
  var interpreterClass = interpreter.createObject(interpreter.FUNCTION);

  if (AClass.prototype != null && AClass.prototype.exposedMethods != null) {
    for (var name in AClass.prototype.exposedMethods) {
  ...
  var constructor = function constructor() {
    var instance = interpreter.createObject(interpreterClass);
    var args = [].concat(Array.prototype.slice.call(arguments)).map(function (argument) {
      return _toNativeData(argument);
    ...
    var declickObject = new (Function.prototype.bind.apply(AClass, [null].concat(_toConsumableArray(args))))();
...
var _toInterpreterInstance = function _toInterpreterInstance(interpreter, instance) {
  var interpreterInstance = interpreter.createObject(interpreter.FUNCTION);
  interpreterInstance.data = instance;

  if (instance.exposedMethods != null) {
    for (var name in instance.exposedMethods) {
...
var data = {
  createInterpreter: function createInterpreter() {
    _interpreter = new _interpreter2.default('', function (interpreter, scope) {
      var name; // at first launch, create and store interpreter instances and classes
  ...
  toInterpreterData: function toInterpreterData(data) {
    return _toInterpreterData(_interpreter, data);
  },
  toNativeData: function toNativeData(data) {
    return _toNativeData(data);
  },
  addClass: function addClass(aClass, name) {
    _classes[name] = aClass;
  },
  addInstance: function addInstance(instance, name) {
  ...
  findInterpreterObject: function findInterpreterObject(name) {
    try {
      var obj = _interpreter.getValueFromScope(name);
  ...
  findInterpreterObjectName: function findInterpreterObjectName(reference) {
    var scope = _interpreter.getScope();
  ...
  deleteInterpreterObject: function deleteInterpreterObject(reference) {
    var scope = _interpreter.getScope();
  ...
  exposeProperty: function exposeProperty(reference, property, propertyName) {
    var scope = _interpreter.getScope();
    ...
    while (scope) {
      for (var name in scope.properties) {
        ...
        if (obj.data === reference) {
          var prop = _interpreter.createObject(null);
...
var _jsInterpreter = _interopRequireDefault(__webpack_require__(/*! js-interpreter */ "./node_modules/js-interpreter/lib/index.js"));
...
var _class =
/*#__PURE__*/
function (_JSInterpreter) {
  _inherits(_class, _JSInterpreter);
  ...
  _createClass(_class, [{
    ...
    key: "executeException",
    value: function executeException(error) {
      // Search for a try statement.
      var state;
      var i = this.stateStack.length - 1;
    ...
    key: "stepRepeatStatement",
    value: function stepRepeatStatement() {
      var state = this.stateStack[this.stateStack.length - 1];
      state.isLoop = true;
      var node = state.node;
    ...
    key: "stepInnerCallExpression",
    value: function stepInnerCallExpression() {
      var state = this.stateStack.pop();
      var args = state.parameters ? state.parameters : state.node.parameters ? state.node.parameters : [];
      this.stateStack.push({
        node: {
          type: 'CallExpression',
          arguments: args
    ...
    key: "appendStatements",
    value: function appendStatements(statements, parameters, callback) {
      var body = statements.map(function (statement) {
    ...
    key: "insertStatements",
    value: function insertStatements(statements, parameters, callback) {
      // Append the new statements
      if (callback !== null) {
        this.stateStack.push({
    ...
    key: "stepInterruptStatement",
    value: function stepInterruptStatement() {
    ...
    key: "stepCallbackStatement",
    value: function stepCallbackStatement() {
      var state = this.stateStack.pop();
      var node = state.node;
    ...
    key: "createCallbackStatement",
    value: function createCallbackStatement(callback) {
      return {
    ...
    key: "createCallStatement",
    value: function createCallStatement(functionStatement) {
      return {
        type: 'InnerCallExpression',
        arguments: [],
        func_: functionStatement,
        loc: functionStatement.node.loc
      };
    }
  }, {
    key: "createFunctionStatement",
    value: function createFunctionStatement(body, parameters) {
      ...
      var statement = this.createFunction(node, this.getScope());
      return statement;
    }
  }, {
    key: "getGlobalScope",
    ...
    key: "setGlobalScope",
    value: function setGlobalScope(aScope) {
    ...
    value: function reset(scope) {
      var emptyAST = _parser.default.parse('');
      ...
      if (scope == null) {
        ...
        this.NUMBER_ZERO.parent = this.NUMBER;
        this.NUMBER_ONE.parent = this.NUMBER;
        this.STRING_EMPTY.parent = this.STRING; // Run the polyfills.

        this.ast = _parser.default.parse(this.polyfills_.join('\n'), this.PARSE_OPTIONS);
        this.polyfills_ = undefined; // Allow polyfill strings to garbage collect.
...
var _data2 = _interopRequireDefault(__webpack_require__(/*! ./data */ "./src/data.js"));

var _scheduler2 = _interopRequireDefault(__webpack_require__(/*! ./scheduler */ "./src/scheduler.js"));

var _parser2 = _interopRequireDefault(__webpack_require__(/*! ./parser */ "./src/parser.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _interpreter = null;
var _default = {
  initialize: function initialize(classes, instances) {
    for (var name in classes) {
      _data2.default.addClass(classes[name], name);
    }

    for (var _name in instances) {
      _data2.default.addInstance(instances[_name], _name);
    }

    _interpreter = _data2.default.createInterpreter();

    _scheduler2.default.initialize(_interpreter);
  },
  getDeclickObjectName: function getDeclickObjectName(reference) {
    if (reference.objectName == null) {
      reference.objectName = _data2.default.findInterpreterObjectName(reference);
    }

    return reference.objectName;
  },
  suspend: function suspend() {
    _scheduler2.default.clear();
  },
  resume: function resume() {
    _scheduler2.default.resume();
  },
  clear: function clear() {
    _data2.default.clear();

    _scheduler2.default.clear();
  },
  executeCode: function executeCode(code) {
    _scheduler2.default.addStatements(_parser2.default.parse(code));
  },
  executeStatements: function executeStatements(statements) {
    _scheduler2.default.addStatements(statements);
  },
  executePriorityCode: function executePriorityCode(code) {
    _scheduler2.default.addPriorityStatements(_parser2.default.parse(code));
  },
  executePriorityStatements: function executePriorityStatements(statements) {
    _scheduler2.default.addPriorityStatements(statements);
  },
  getStatements: function getStatements(code) {
    return _parser2.default.parse(code);
  },
  getLastValue: function getLastValue() {
    return _scheduler2.default.getLastValue();
  },
  reset: function reset() {
    _scheduler2.default.clear();
...
var _acorn = __webpack_require__(/*! acorn */ "./node_modules/acorn/dist/acorn.es.js");
...
var _repeatKeyword = 'repeat';
_acorn.tokTypes.repeat = _acorn.keywordTypes.repeat = new _acorn.TokenType('repeat', {
  isLoop: true,
  name: 'repeat'
}); // acorn plugin to handle repeat keyword and add additional info

_acorn.plugins.declick = function (instance) {
  // add repeat keyword
  var keywordRegex = instance.keywords.toString();
  instance.keywords = new RegExp(keywordRegex.slice(1, -3) + '|' + _repeatKeyword + ')$'); // set strict mode

  instance.strict = true; // detect repeat keyword

  instance.extend('parseStatement', function (superFunction) {
    return function (declaration, topLevel, exports) {
      if (this.type === _acorn.tokTypes.repeat) {
  ...
  instance.extend('parseRepeatStatement', function (superFunction) {
    return function (node) {
      this.next();
      this.expect(_acorn.tokTypes.parenL);

      if (this.type === _acorn.tokTypes.parenR) {
        node.count = null;
      } else {
  ...
  instance.extend('finishNode', function (superFunction) {
...
var _default = {
  setRepeatKeyword: function setRepeatKeyword(name) {
    _repeatKeyword = name;
    _acorn.tokTypes.repeat = _acorn.keywordTypes[_repeatKeyword] = new _acorn.TokenType('repeat', {
      isLoop: true,
      name: 'repeat'
    });
  },
  parse: function parse(input, programName) {
    if (programName) {
    ...
    return (0, _acorn.parse)(input, _options);
  }
...
var _error = _interopRequireDefault(__webpack_require__(/*! ./error */ "./src/error.js"));
...
var _stepCount = 0;
var _running = false;
var _priorityStatementsAllowed = true;
var _interpreter = null;
var _priorityInterpreter = null;
var _priorityStep = false;
...
var _nextStep = function _nextStep() {
  try {
    ...
    var state, error;
    var interpreter = _priorityStep ? _priorityInterpreter : _interpreter;

    if (!(err instanceof _error.default)) {
      error = new _error.default(err);
...
var _stop = function _stop(scope) {
  _running = false;
...
var _appendStatements = function _appendStatements(interpreter, statements, parameters, callback) {
  if (parameters != null) {
    for (var i = 0; i < parameters.length; i++) {
      parameters[i] = _data.default.toInterpreterData(parameters[i]);
    }
  }
...
var _insertStatements = function _insertStatements(interpreter, statements, parameters, callback) {
  if (typeof parameters !== 'undefined') {
    for (var i = 0; i < parameters.length; i++) {
      parameters[i] = _data.default.toInterpreterData(parameters[i]);
    }
...
var _default = {
  ...
  addStatements: function addStatements(statements, parameters, callback) {
  ...
  allowPriorityStatements: function allowPriorityStatements() {
    _priorityStatementsAllowed = true;
  },
  refusePriorityStatements: function refusePriorityStatements() {
    _priorityStatementsAllowed = false;
  },
  addPriorityStatements: function addPriorityStatements(statements, parameters, callback) {
  ...
  insertStatements: function insertStatements(statements, parameters, callback) {
    _insertStatements(_interpreter, statements, parameters, callback);

    _start();
  },
  // TODO: voir si on s'en sert
  insertBlockStatement: function insertBlockStatement(blockStatement) {
    _interpreter.insertBlock(blockStatement);

    _start();
  },
  // TODO: est-ce qu'on gère le cas d'un Programme
  insertPriorityStatements: function insertPriorityStatements(statements, parameters, callback) {
    if (_priorityStatementsAllowed) {
  ...
  initialize: function initialize(interpreter) {
    _interpreter = interpreter;
    _priorityInterpreter = new _interpreter2.default('');
...
//# sourceMappingURL=declick-runtime.js.map