define([
  ...
  function TRuntime() {
    ...
    this.load = function(callback) {
      ...
      TEnvironment.log("loading Arduino agent");
      this.loadArduinoAgent()
          ...
          for (var i = 0; i < val.conditions.length; i++) {
            ...
            switch (condition) {
              case "3d":
                if (!is3DSupported) {
                  console.log(
                    "skipping addition of object " + key + ": 3D not supported"
                  );
                  addObject = false;
                }
                break;
        ...
        if (addObject) {
          ...
          if (typeof val.translations[language] !== "undefined") {
            ...
            require([lib], function(aClass) {
              ...
              TI18n.internationalize(
                aConstructor,
                parents,
                language,
                function() {
                  TEnvironment.log(
                    "Declaring translated object '" + translatedName + "'"
                  );
                  if (instance) {
    ...
    this.loadArduinoAgent = function() {
      var script = document.createElement("script");
      script.src =
        "js/libs/arduinoCreateAgentDaemon/ArduinoCreateAgentDaemon.js";
    ...
    this.getClassTranslatedMethods = function(className) {
      ...
      if (
        theClass &&
        typeof theClass.prototype.translatedMethodsDisplay !== "undefined"
      ) {
    ...
    this.getTObjectTranslatedMethods = function(objectName) {
      ...
      if (
        theObject &&
        typeof theObject.translatedMethodsDisplay !== "undefined"
      ) {
    ...
    this.executeStatementsNow = function(
      statements,
      parameters,
      log,
      callback
