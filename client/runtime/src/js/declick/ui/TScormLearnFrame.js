define(['ui/TComponent', 'jquery', 'ui/TLearnCanvas', 'ui/TLearnEditor', 'TRuntime', 'TEnvironment', 'TExerciseProject', 'TError', 'prism', 'TLink', 'split-pane'], function (TComponent, $, TLearnCanvas, TLearnEditor, TRuntime, TEnvironment, TExerciseProject, TError, Prism, TLink) {
    function TScormLearnFrame(callback) {
        ...
        var closeExercise = function() {
          window.parent.postMessage({action:"close"}, "*");
        }

        var getCurrentInput = function() {
          return editor.getValue() === "" ? $('#tlearnframe-text-input').val() : editor.getValue();
        }

        var saveExercise = function() {
          window.parent.postMessage({action:"save", data:getCurrentInput()}, "*");
        }
        ...
        var loadExercise = function(input) {
          $('#tlearnframe-text-input')[0].value = input;
        ...
        var manageMessage = function(event) {
          ...
          if (message.action !== undefined) {
            if (message.action === "load" && message.data !== undefined) {
              loadExercise(message.data);
            }
          }
        }

        var approveExercise = function(close, save) {
          var parameters = {action:"approve", close:close, save:save};
          if (save) {
            parameters.data = getCurrentInput();
          }
    ...
    TScormLearnFrame.prototype = Object.create(TComponent.prototype);
    TScormLearnFrame.prototype.constructor = TScormLearnFrame;

    return TScormLearnFrame;
});
