...
function makeTest(test) {
  if (typeof test === 'string') return type => type === test
...
base.ExpressionStatement = base.ParenthesizedExpression = (node, st, c) =>
...
base.ThrowStatement = base.SpreadElement = (node, st, c) =>
...
export function walk1(ast, comments, block_loc, options) {
  ...
  var newNode = function(name, attrs, text, ast_node) {
    ...
    if (name === 'block') {
      ...
      while (
        i < comments.length &&
        comments[i].loc.start.line <= ast_node.loc.start.line
      ) {
        if (first) {
          first = false
  ...
  funcs.Statement = function(node, st, c) {
    ...
    if (!options.joinTopBlocks && current_node === root_node) {
      ...
      current_node.children[lastChild - 1].setAttribute(
        'y',
        (lastChild * 10).toString(),
      )
    } else {
  ...
  funcs.SwitchStatement = (node, st, c) => {
    ...
    var mutation1 = newNode('mutation', {
      items: node.cases.length + 1 - hasDefault,
    }) // TODO: Take out +1 when items count corrected on AddSub block
    block1.appendChild(mutation1)
  ...
  funcs.ThrowStatement = (node, st, c) => {
  ...
  funcs.FunctionExpression = funcs.ArrowFunctionExpression =
  ...
  funcs.NewExpression = (node, st, c) => {
    ...
    var mutation1 = newNode('mutation', {
      items: node.arguments.length + 1,
      names: '',
    }) // TODO: Take out +1 when AddSub... is fixed on the actual length required.
  ...
  funcs.MemberExpression = (node, st, c) => {
    ...
    if (is_call) {
      ...
      if (node.computed) {
        ...
        block1 = newNode('block', { type: 'bi_field_return' }, '', node)
        block1.appendChild(
          newNode('field', { name: 'NAME' }, node.property.name),
        )
      }
      var value1 = newNode('value', { name: 'chain' })
      block1.appendChild(value1)
  ...
  funcs.ImportSpecifier = function(node, st, c) {
    ...
    if (node.imported.name === node.local.name) {
      ...
      block1.appendChild(
        newNode('field', { name: 'input' }, node.imported.name),
      )
    ...
    if (node.value instanceof RegExp) {
      ...
      switch (typeof node.value) {
        case 'number':
          ...
          block1.appendChild(
            newNode('field', { name: 'NUM' }, node.value.toString()),
          )
          break
        case 'boolean':
    ...
    switch (node.kind) {
      case 'set':
        ...
        if (node.computed === false) {
          ...
          block1.appendChild(
            newNode('field', { name: 'name' }, '[' + node.key.name + ']'),
          )
        }
        block1.appendChild(
          newNode('field', { name: 'val' }, node.value.params[0].name),
        )
  ...
  if (last_comment < comments.length) {
    current_node.appendChild(
      newNode('block', { type: 'bi_comment' }, '', fake_last_line_node),
    )
  }
