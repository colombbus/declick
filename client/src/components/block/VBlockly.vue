...
const VBlockly = {
  ...
  mounted() {
    const locale = 'fr'
    const i18nMsg = require(`../../../node_modules/blockly/msg/${locale}`)

    this.translate(i18nMsg)
    ...
    this.loadToolbox().then(toolbox => {
      ...
      categories.forEach(c =>
        c.setAttribute(
          'name',
          categoriesFR[c.getAttribute('name')] || c.getAttribute('name'),
        ),
      )
  ...
  methods: {
    translate(msg) {
      delete Blockly['Msg']
      Blockly['Msg'] = Object.assign(msg, Blockly['Msg'])
    ...
    loadBlocks() {
      ...
      const promise = fetch(this.blocks).then(resp => {
        return resp.json()
      })
      return promise
    },
...
export { VBlockly as default, Blockly }
</script>
