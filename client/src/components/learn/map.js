...
function DeclickMap() {
  ...
  var initView = function(canvasId) {
    ...
    var handleScroll = function() {
      ...
      if (newZoom < 1) {
        ...
        var newCenter = scrollPoint.add(
          targetCenter.subtract(scrollPoint).divide(newZoom / targetZoom),
        )
    ...
    paper.view.onFrame = function(event) {
      ...
      if (changeCurrent) {
        ...
        if (vector.length > step) {
          ...
          current.position = new paper.Point(targetCurrent)
          changeCurrent = false
          if (currentCallback) {
            currentCallback()
          }
  ...
  var initSymbols = function(currentSVG, callback) {
    ...
    var pChapterValidatedOuter = new paper.Path.Circle(
      new paper.Point(0, 0),
      12,
    )
    ...
    var pChapterValidatedInner = new paper.Path.Circle(
      new paper.Point(0, 0),
      10,
    )
  ...
  var setTargetCenter = function(center, fast) {
    ...
    changeCenter = true
    movementSpeed =
      paper.view.center.getDistance(targetCenter) / animationDuration
    if (fast) {
  ...
  var displaySteps = function() {
    ...
    var placeSymbol = function(index, curve, length, last) {
      ...
      if (chapter) {
        ...
        if (
          typeof steps[index + 1] !== 'undefined' &&
          !steps[index + 1].chapter
        ) {
          hasSubItems = true
        }
        if (hasSubItems) {
