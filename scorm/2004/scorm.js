...
function ScormProcessGetLearnerResponse(id) {
  var count = ScormProcessGetValue("cmi.interactions._count");
  for (var i = 0; i<count;i++) {
    var interactionId = ScormProcessGetValue("cmi.interactions."+i+".id");
    if (interactionId === id) {
      return ScormProcessGetValue("cmi.interactions."+i+".learner_response");
    }
  }
  return undefined;
}

function ScormProcessSetLearnerResponse(id, response) {
  var count = ScormProcessGetValue("cmi.interactions._count");
  for (var i = 0; i<count;i++) {
    var interactionId = ScormProcessGetValue("cmi.interactions."+i+".id");
    if (interactionId === id) {
      ScormProcessSetValue("cmi.interactions."+i+".learner_response", response);
      return;
    }
  }
  ScormProcessSetValue("cmi.interactions."+count+".id", id);
  ScormProcessSetValue("cmi.interactions."+count+".type", "long-fill-in");
  ScormProcessSetValue("cmi.interactions."+count+".learner_response", response);
  return;
}
